(function($){ 
    ...
    var methods = {
        ...
        init: function( options ) {
            ...
            var settings = {
                ...
                'strokewidth': 4,
                'handlewidth': 4,
                'increments': 100,//XXX Hacked By Kyle
                'minrad': 10
            }
    ...
    function drawGrid( $sg, canvas ) {
        ...
        for ( var x=0; x<increments; x+=10 ) { //XXX hacked by kyle
 ...
 dy - adjusty ) + 'px'  } );
    ...
    function drawDataSet( $sg, canvas, layerdata ) {
        ...
        for ( var x=0; x< data.settings.fields.length ; x++ ) {
            ...
            curvemage = (curvemage*.15) * (0.15*( ((layerdata.data[x] + 2)/increments + 4)) );
            var curvemaga = Math.sqrt( (ncoords.x-pcoords.x) * (ncoords.x-pcoords.x) + (ncoords.y-pcoords.y) * (ncoords.y-pcoords.y)  );
            curvemaga = (curvemaga*.15) * (0.15*(((layerdata.data[(x+1) %data.settings.fields.length] + 2)/increments + 4)) );
